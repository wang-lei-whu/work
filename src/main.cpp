...
int main()
{
     ...
     //维度和定义域
     const int dim = 16;
     const MatrixXd lb = MatrixXd::Constant(1, dim, -2);
     const MatrixXd ub = MatrixXd::Constant(1, dim, 2);
     ...
     for (int i = 0; i < 1; i++)
     {
          ...
          PSO pso(toolFunc::rosenBrock, dim, 200000, 100, lb, ub, 0.8, 0.3, 0.6, 0.6, false);
          pso.run();
          cout << "Best value:\n"
               << pso.gbest_y
               << "\nX for best value:\n"
